...
def top_tfidf_terms_sparse(vec, vocab, top_n=5):
    """Return the top N terms with highest TF-IDF values from a sparse vector."""
    inv_vocab = {i:t for t,i in vocab.items()}
    return sorted([(inv_vocab[i], v) for i, v in vec.items()], key=lambda x: x[1], reverse=True)[:top_n]
